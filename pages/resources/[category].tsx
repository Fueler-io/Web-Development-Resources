...
export default function DashboardWithFilter() {
  ...
  const {data, isLoading, error} = useQuery(['resources', router.query.category], fetchResources);
  return (
    <div>
      <ul
        role="list"
      ...
      >
        <li className="text-white">{JSON.stringify(data)}</li>
        </ul>
    </div>
  );
}
...
export const  getStaticProps: GetStaticProps = async (context) => {
  ...
  await queryClient.prefetchQuery(['resources', context.params && context.params.category], fetchResources);
